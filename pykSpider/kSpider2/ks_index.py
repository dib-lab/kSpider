...
def validate_names(ctx, names_file):
    ...
    ctx.obj.Logger.INFO("validating names file..")

    with open(names_file) as names:
        for i, line in enumerate(names, 1):
            if len(line.strip().split("\t")) != 2:
                ctx.obj.Logger.ERROR(f"invalid names line detected at L{i}: '{line.strip()}'")

    
    # elif reading_mode == "SKIPMERS":
    #     m, n, k = value
    #     if n < 1 or n < m or k < m or k % m != 0:
    #         raise click.BadParameter("Invalid skip-mer shape!\nConditions: 0 < m <= n < k & k must be multiple of m")
    #     return value

    # elif reading_mode in ["PROTEIN", "DAYHOFF"]:
    #     if value > 11:
    #         raise click.BadParameter("Invalid kSize. Max kmer size of protein sequences is 11")
    #     return value
...
def kmers(ctx, fasta_file, names_file, kSize, output_prefix, chunk_size, strand_specific):
    ...
    if strand_specific:
        kSpider_internal.index_kmers_nonCanonical(kSize, fasta_file, names_file, chunk_size, output_prefix)
    else:
        kSpider_internal.index_kmers(kSize, fasta_file, names_file, chunk_size, output_prefix)
...
def skipmers(ctx, fasta_file, names_file, skipmers_kSize, skipmers_m, skipmers_n, chunk_size, output_prefix):
    ...
    ctx.obj.INFO("Indexing has begun, please wait ....")
    kSpider_internal.index_skipmers(skipmers_m, skipmers_n, skipmers_kSize, fasta_file, names_file, chunk_size, output_prefix)
...
def protein(ctx, fasta_file, names_file, kSize, output_prefix, chunk_size, dayhoff):
    ...
    if dayhoff:
        kSpider_internal.index_dayhoff(kSize, fasta_file, names_file, chunk_size, output_prefix)
    else:
        kSpider_internal.index_dayhoff(kSize, fasta_file, names_file, chunk_size, output_prefix)