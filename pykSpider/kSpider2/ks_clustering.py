...
class Clusters:
    ...
    def __init__(self, logger_obj, index_prefix, cut_off_threshold, dist_type):
        ...
        self.output = index_prefix + f"_kSpider_clusters_{cut_off_threshold}%.tsv"
    ...
    def cluster_graph(self):
        ...
        with open(self.output, 'w') as CLUSTERS:
            ...
            #     single_components += 1
            #     continue
                named_component = [self.names_map[node + 1] for node in component]
...
@click.option('-d', '--dist-type', "distance_type", required=False, default="max_cont", type=click.STRING, help="select from ['min_containment', 'avg_containment', 'max_containment', 'ani']")
@click.pass_context
def main(ctx, index_prefix, cutoff, distance_type):
    """Sequence clustering."""
    cutoff = float(cutoff) * 100
    kCl = Clusters(logger_obj=ctx.obj, index_prefix=index_prefix, cut_off_threshold=cutoff, dist_type=distance_type)
